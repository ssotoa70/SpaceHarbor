#!/usr/bin/env python3
"""AssetHarbor Deployment CLI — guided wizard for deploying the full stack.
...
MIGRATIONS = [
    "001_vfx_hierarchy.ts",
    "002_episode_task.ts",
    "003_review_status.ts",
    "004_version_frame_handles.ts",
    "005_materialx_materials.ts",
]

MIGRATION_DIR = Path("services/control-plane/src/db/migrations")
...
class VastCredentials:
    ...
    def secrets(self) -> list[str]:
        return [self.secret_key, self.api_token, self.access_key]
...
def validate_trino_endpoint(url: str) -> tuple[bool, str]:
    ...
    try:
        ...
        req.add_header("User-Agent", "AssetHarbor-Deploy/1.0")
...
def prompt_api_key() -> str:
    """Generate or accept an API key for AssetHarbor."""
    print()
    idx = prompt_choice(
        "API Key",
...
def generate_env(config: DeployConfig) -> str:
    ...
    if config.mode == "local":
        lines.append("ASSETHARBOR_PERSISTENCE_BACKEND=local")
        lines.append("ASSETHARBOR_VAST_STRICT=false")
    else:
        lines.append("ASSETHARBOR_PERSISTENCE_BACKEND=vast")
        lines.append("ASSETHARBOR_VAST_STRICT=true")

    lines.append(f"ASSETHARBOR_API_KEY={config.api_key}")
...
def run_migrations(config: DeployConfig) -> bool:
    """Run database migrations against VAST Trino."""
    migration_base = PROJECT_ROOT / MIGRATION_DIR
    ...
    env = os.environ.copy()
    creds = config.credentials
    env["VAST_TRINO_ENDPOINT"] = creds.trino_endpoint
    env["VAST_ACCESS_KEY"] = creds.access_key
    env["VAST_SECRET_KEY"] = creds.secret_key

    for mig in MIGRATIONS:
        mig_path = migration_base / mig
        if not mig_path.exists():
            print(f"  {warn_mark()} {dim(mig)} — not found, skipping")
            log.warning(f"Migration file not found: {mig_path}")
            continue
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"\r  {check_mark()} {name}     ")
                log.info(f"Migration {name}: OK")
            else:
                print(f"\r  {cross_mark()} {name}     ")
                log.error(f"Migration {name} failed: {result.stderr}")
                print(f"    {dim(result.stderr[:200])}")
                return False
        except subprocess.TimeoutExpired:
            print(f"\r  {cross_mark()} {name} — timed out")
            log.error(f"Migration {name} timed out")
            return False
...
def config_from_env_and_file(args: argparse.Namespace) -> DeployConfig:
    ...
    cfg = DeployConfig(
        mode=args.mode or os.environ.get("ASSETHARBOR_DEPLOY_MODE", "local"),
    ...
    cfg.api_key = extra.get("api_key", os.environ.get("ASSETHARBOR_API_KEY", ""))
    if not cfg.api_key:
...
def run_teardown(args: argparse.Namespace) -> int:
    ...
    if not args.force and not args.non_interactive:
        if not prompt_yes_no("Stop all AssetHarbor containers?", default=True):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="deploy",
        description="AssetHarbor Deployment CLI — deploy the full stack with a guided wizard.",
