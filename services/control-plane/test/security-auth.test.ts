...
function withApiKeyEnv<T>(apiKey: string, run: () => Promise<T>): Promise<T> {
  const previous = process.env.SPACEHARBOR_API_KEY;
  process.env.SPACEHARBOR_API_KEY = apiKey;

  return run().finally(() => {
    if (previous === undefined) {
      delete process.env.SPACEHARBOR_API_KEY;
      return;
    }
    process.env.SPACEHARBOR_API_KEY = previous;
  });
}
