...
test("processVastFunctionCompletion routes exr_inspector events", () => {
  ...
  const result = processVastFunctionCompletion(persistence, event, "exr_inspector", CTX);
...
test("processVastFunctionCompletion routes mtlx_parser events", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent({ metadata: { material_name: "HeroSkin" } });
  const result = processVastFunctionCompletion(persistence, event, "mtlx_parser", CTX);

  assert.equal(result.accepted, true);
  assert.equal(result.action, "mtlx_parsed");
});

test("processVastFunctionCompletion routes otio_parser events", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent({ metadata: { timeline_name: "Edit_v3" } });
  const result = processVastFunctionCompletion(persistence, event, "otio_parser", CTX);

  assert.equal(result.accepted, true);
  assert.equal(result.action, "otio_parsed");
});

test("processVastFunctionCompletion routes oiio_proxy_generator events", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent();
  const result = processVastFunctionCompletion(persistence, event, "oiio_proxy_generator", CTX);

  assert.equal(result.accepted, true);
  assert.equal(result.action, "proxy_generated");
});

test("processVastFunctionCompletion routes scanner events", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent();
  const result = processVastFunctionCompletion(persistence, event, "scanner", CTX);

  assert.equal(result.accepted, true);
  assert.equal(result.action, "scan_completed");
});

test("processVastFunctionCompletion handles unknown function", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent();
  const result = processVastFunctionCompletion(persistence, event, "new_unknown_fn", CTX);

  assert.equal(result.accepted, true);
  assert.equal(result.action, "unknown_function");
});

test("processVastFunctionCompletion deduplicates events", () => {
  const persistence = new LocalPersistenceAdapter();

  const event = makeEvent();

  const first = processVastFunctionCompletion(persistence, event, "scanner", CTX);
  assert.equal(first.action, "scan_completed");

  const second = processVastFunctionCompletion(persistence, event, "scanner", CTX);
