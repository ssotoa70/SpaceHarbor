...
describe("processProxyGeneratedEvent", () => {
  it("updates asset thumbnail_url and proxy_url in metadata", async () => {
    ...
    const updated = await persistence.getAssetById(assetId);
  ...
  it("is a no-op for unknown asset_id", async () => {
    const persistence = new LocalPersistenceAdapter();
    // Should not throw
    await processProxyGeneratedEvent(
      {
