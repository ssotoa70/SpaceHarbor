...
test("extended asset model: VFX metadata fields are preserved through create and update", () => {
  ...
  const { asset } = adapter.createIngestAsset(
  ...
  const initial = adapter.getAssetById(asset.id);
  ...
  const updated = adapter.updateAsset(
    asset.id,
...
test("extended asset model: version tracking with parent chain", () => {
  const adapter = createPersistenceAdapter("local");

  // Create v001
  const { asset: assetV1 } = adapter.createIngestAsset(
    { title: "shot_010_comp_v001.exr", sourceUri: "s3://renders/shot_010_comp_v001.exr" },
  ...
  const { asset: assetV2 } = adapter.createIngestAsset(
  ...
  const v1 = adapter.getAssetById(assetV1.id);
  const v2 = adapter.getAssetById(assetV2.id);
...
test("extended asset model: integrity tracking (checksum + verified_at)", () => {
  const adapter = createPersistenceAdapter("local");
...
test("extended asset model: metadata update preserves version and integrity", () => {
  const adapter = createPersistenceAdapter("local");
  ...
  const final = adapter.getAssetById(asset.id);
...
test("extended asset model: metadata merges (partial update does not clobber)", () => {
  const adapter = createPersistenceAdapter("local");
...
test("extended asset model: updateAsset returns null for non-existent asset", () => {
  const adapter = createPersistenceAdapter("local");

  const result = adapter.updateAsset(
    "non-existent-id",
    { metadata: { codec: "test" } },
...
test("extended asset model: getAssetById returns null for non-existent asset", () => {
  const adapter = createPersistenceAdapter("local");

  const result = adapter.getAssetById("non-existent-id");
  assert.equal(result, null, "Should return null for non-existent asset");
});

test("extended asset model: backward compatibility (existing tests pattern)", () => {
  const adapter = createPersistenceAdapter("local");

  // Standard ingest flow should still work with no extended fields
  const { asset, job } = adapter.createIngestAsset(
