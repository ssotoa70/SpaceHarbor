...
export async function registerEventsRoute(
  ...
  app.post(withPrefix("", "/events"), {
    ...
    const result = await processAssetEvent(persistence, normalizeLegacyEvent(request.body), {
  ...
  app.post(withPrefix("/api/v1", "/events"), {
    ...
    const result = await processAssetEvent(persistence, normalizeCanonicalEvent(request.body), {
      correlationId: resolveCorrelationId(request)
    }, {
