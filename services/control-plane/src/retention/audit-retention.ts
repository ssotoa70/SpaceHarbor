...
export function resolveAuditRetentionConfig(env: NodeJS.ProcessEnv = process.env): AuditRetentionConfig {
  const enabled = (env.SPACEHARBOR_AUDIT_RETENTION_ENABLED ?? "true").toLowerCase() !== "false";
  const rawMode = (env.SPACEHARBOR_AUDIT_RETENTION_MODE ?? "dry-run").toLowerCase();
  const mode: AuditRetentionMode = rawMode === "apply" ? "apply" : "dry-run";

  return {
    enabled,
    mode,
    retentionDays: parsePositiveInt(env.SPACEHARBOR_AUDIT_RETENTION_DAYS, DEFAULT_RETENTION_DAYS),
    intervalSeconds: parsePositiveInt(env.SPACEHARBOR_AUDIT_RETENTION_INTERVAL_SECONDS, DEFAULT_INTERVAL_SECONDS),
    maxDeletePerRun: parseOptionalPositiveInt(env.SPACEHARBOR_AUDIT_RETENTION_MAX_DELETE_PER_RUN)
  };
}
  ...
  const runNow = async (now: Date = new Date()): Promise<AuditRetentionRunSummary> => {
    ...
    try {
      ...
      if (config.mode === "dry-run") {
        const preview = await persistence.previewAuditRetention(cutoffIso);
        return {
          mode: "dry-run",
          cutoffIso,
      ...
      const result = await persistence.applyAuditRetention(cutoffIso, config.maxDeletePerRun);
      return {
        mode: "apply",
        cutoffIso,
