...
const TARGET_CONFIG: Array<{ target: OutboundTarget; envKey: string }> = [
  { target: "slack", envKey: "SPACEHARBOR_WEBHOOK_SLACK_URL" },
  { target: "teams", envKey: "SPACEHARBOR_WEBHOOK_TEAMS_URL" },
  { target: "production", envKey: "SPACEHARBOR_WEBHOOK_PRODUCTION_URL" }
];
...
export function resolveOutboundConfig(env: NodeJS.ProcessEnv = process.env): OutboundConfig {
  const strictMode = parseStrictBoolean(env.SPACEHARBOR_WEBHOOK_STRICT_MODE);
  const signingSecret = (env.SPACEHARBOR_WEBHOOK_SIGNING_SECRET ?? "").trim();
  ...
  if ((strictMode || targets.length > 0) && !signingSecret) {
    throw new OutboundConfigError("missing required config: SPACEHARBOR_WEBHOOK_SIGNING_SECRET");
  }
...
function parseStrictBoolean(input: string | undefined): boolean {
  ...
  throw new OutboundConfigError("invalid SPACEHARBOR_WEBHOOK_STRICT_MODE value (expected true/false)");
}
