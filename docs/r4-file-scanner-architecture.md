...
│      s3://assetharbor-renders/projects/NOVA/SEQ_010/SH040/render/v001/  │
└──────────────────────────────┬──────────────────────────────────────────┘
                               │  S3 PUT (via NFS-to-S3 gateway or direct S3)
                               ▼
┌──────────────────────────────────────────────────────────────────────────┐
│  VAST Element Store (S3-native bucket view: assetharbor-renders)         │
...
└──────────────────────────────┬───────────────────────────────────────────┘
                               ...
                               ▼
┌──────────────────────────────────────────────────────────────────────────┐
│  AssetHarbor Control-Plane  (Fastify)                                     │
│                                                                           │
...
AssetHarbor expects renders to follow this S3 object key structure:
...
- Unsupported extensions are silently skipped (only `.exr`, `.mov`, `.dpx`, `.audio`, `.vdb`, `.usd` trigger ingestion)
...
| Source view | `assetharbor-renders` (S3-native bucket) |
...
```sql
-- Find all render files not yet in AssetHarbor
SELECT c.name, c.parent_path, c.mtime, c.size
FROM vast."assetharbor/production".catalog c
WHERE c.search_path = '/projects/'
  AND c.extension IN ('exr', 'mov', 'dpx')
  AND c.parent_path LIKE '%/render/%'
  AND NOT EXISTS (
    SELECT 1 FROM vast."assetharbor/production".assets a
...
DLQ items (repeated failures) should be routed to the existing AssetHarbor DLQ table in VastDB and surface in the admin UI (existing DLQ route at `GET /queue/dlq`).
...
def poll_catalog(event, context):
    ...
    new_files = trino.query("""
        ...
        FROM vast."assetharbor/production".catalog
        WHERE search_path = '/projects/'
          AND extension IN ('exr', 'mov', 'dpx')
          AND parent_path LIKE '%/render/%'
          AND mtime > ?
          AND NOT EXISTS (
              SELECT 1 FROM vast."assetharbor/production".assets
...
|---|---|---|
| `ScannerFunction` (new) | `ElementCreated` on render files | Detect new renders → create asset/version in AssetHarbor |
...
docker build -t ghcr.io/dev-ss/assetharbor/scanner-function:latest .
docker push ghcr.io/dev-ss/assetharbor/scanner-function:latest

# 2. Register the function in VAST DataEngine (via VAST CLI or UI)
vast dataengine function create \
  --name scanner-function \
  --image ghcr.io/dev-ss/assetharbor/scanner-function:latest \
  --env VAST_TRINO_ENDPOINT=$VAST_TRINO_ENDPOINT \
  --env ASSETHARBOR_CONTROL_PLANE_URL=$ASSETHARBOR_CONTROL_PLANE_URL
...
vast dataengine trigger create \
  ...
  --source-view assetharbor-renders \
  --prefix "projects/" \
  --suffix ".ready" \
  --function scanner-function

# 4. Verify (write a test file and check logs)
echo "" | aws s3 cp - s3://assetharbor-renders/projects/TEST/SEQ_001/SH001/render/v001/.ready
