...
SpaceHarbor's production persistence layer uses VAST Database (Trino SQL). However, developers need to run the system locally without access to a VAST cluster for frontend development, testing, and rapid iteration.
...
3. **Dual-mode persistence with a `PersistenceAdapter` interface** — production uses `VastPersistenceAdapter` (Trino), development uses `LocalPersistenceAdapter` (in-memory).
...
- **`VastPersistenceAdapter`:** Trino SQL queries for production. Supports CAS (compare-and-swap) for optimistic concurrency.

The active adapter is selected by `SPACEHARBOR_PERSISTENCE_BACKEND` (default: `local`).

When `SPACEHARBOR_VAST_FALLBACK_TO_LOCAL=true`, the VAST adapter automatically falls back to local behavior if Trino is unreachable. Fallback events are surfaced in the audit log with `VAST_FALLBACK` signal codes.
...
**Trade-offs:**
- `LocalPersistenceAdapter` does not enforce SQL semantics (no transactions, no constraints). Bugs that only manifest under Trino may not surface in local mode.
