...
1. Copy `.env.example` to `.env` and set:
   - `SPACEHARBOR_PERSISTENCE_BACKEND`
   - `SPACEHARBOR_VAST_STRICT` (recommended `true` for VAST-backed deployments)
   - `SPACEHARBOR_VAST_FALLBACK_TO_LOCAL` (`true` for continuity, `false` for strict fail-fast)
   - VAST endpoints and token
   - optional API keys (`SPACEHARBOR_API_KEY`, `CONTROL_PLANE_API_KEY`, `VITE_API_KEY`)
   - webhook outbound integration config (optional):
     ...
     - `SPACEHARBOR_WEBHOOK_STRICT_MODE` (`true`/`false`)
2. Run `docker compose up --build` from `SpaceHarbor/`.
3. Verify:
...
- Deliveries include HMAC signature header (`x-spaceharbor-signature`) and timestamp (`x-spaceharbor-timestamp`).
...
- Strict mode: with `SPACEHARBOR_VAST_STRICT=true` and `SPACEHARBOR_VAST_FALLBACK_TO_LOCAL=false`, VAST workflow client failures fail-fast.
- Continuity mode: with `SPACEHARBOR_VAST_FALLBACK_TO_LOCAL=true`, client failures fall back to local adapter behavior.
...
- Configure with environment variables:
  - `SPACEHARBOR_AUDIT_RETENTION_ENABLED` (`true`/`false`, default `true`)
  - `SPACEHARBOR_AUDIT_RETENTION_MODE` (`dry-run`/`apply`, default `dry-run`)
  - `SPACEHARBOR_AUDIT_RETENTION_DAYS` (default `90`)
  - `SPACEHARBOR_AUDIT_RETENTION_INTERVAL_SECONDS` (default `3600`)
  - `SPACEHARBOR_AUDIT_RETENTION_MAX_DELETE_PER_RUN` (optional safety cap)
...
4. Keep `SPACEHARBOR_AUDIT_RETENTION_MAX_DELETE_PER_RUN` set during initial rollout.
...
- [Disaster Recovery](disaster-recovery.md) — RTO/RPO targets, recovery procedures for major incidents
- [Troubleshooting](troubleshooting.md) — decision trees for the top 5 failure scenarios
- [Monitoring Setup](monitoring-setup.md) — observable endpoints, Prometheus config, Grafana dashboards
